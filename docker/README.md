...
To disable this image behavior, simply override the container command or entrypoint to run `dockerd` directly (`... arm32v6/docker:dind dockerd ...` or `... --entrypoint dockerd arm32v6/docker:dind ...`).
...
The `arm32v6/docker` images come in many flavors, each designed for a specific use case.

**Note:** The `arm32v6/docker:stable`, `arm32v6/docker:test`, and related "channel" tags have been deprecated since June 2020 (see [docker-library/docker#179](https://github.com/docker-library/docker/pull/179)) and have not been updated since December 2020 (when Docker 20.10 was released). Suggested alternatives are below. `X` is a placeholder for the version; see the supported tags list for the current set of tags.

-	`arm32v6/docker:stable` ⏩ `arm32v6/docker:latest`, `arm32v6/docker:dind`, `arm32v6/docker:X`, `arm32v6/docker:cli`, etc
-	`arm32v6/docker:test` ⏩ `arm32v6/docker:rc`, `arm32v6/docker:rc-dind`, `arm32v6/docker:X-rc`, `arm32v6/docker:rc-cli`, etc (only updated when there is an active pre-release; will not point to the same thing as `latest`)

## `arm32v6/docker:<version>-cli`
...
## `arm32v6/docker:<version>`, `arm32v6/docker:<version>-dind`
