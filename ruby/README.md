...
```console
$ docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app mips64le/ruby:3.3 bundle install
```
...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp mips64le/ruby:3.3 ruby your-daemon-or-script.rb
```
